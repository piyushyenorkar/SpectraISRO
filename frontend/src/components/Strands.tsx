...
void main() {
  ...
  // Calculate horizontal position from -1.0 (left edge) to 1.0 (right edge)
  float nx = (gl_FragCoord.x / uResolution.x) * 2.0 - 1.0;
  // This guarantees the taper fade-out happens exactly at the browser margins
  float env = pow(max(cos(nx * PI * 0.5), 0.0), uTaper);
  ...
  for (int i = 0; i < ${MAX_STRANDS}; i++) {
    ...
    // Add a beautiful sagging 'U' shape so it hangs downwards below the text!
    // cos(nx * PI * 0.5) is 1.0 in the center and 0.0 at the extreme left/right.
    float sag = cos(nx * PI * 0.5) * 0.09; 
    y -= sag;
...
export default function Strands({
  ...
  return <div ref={ctnDom} className={`relative w-full h-full bg-transparent ${className}`} style={{ width: '100%', height: '100%', position: 'relative', ...style }} />;
}
